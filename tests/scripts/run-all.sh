...
EXT_PIDS=()
for label in ash-noni ash-cook jline-smoke ash-jline ash-applet real-coreutils real-heavy env-inherit token-rotate instance-warn jlink-modules guestjob-quote guest-launch sshkeys; do
...
# 結果を元の順序で表示・集計
for label in ash-noni ash-cook jline-smoke ash-jline ash-applet dist-smoke real-coreutils real-heavy env-inherit token-rotate instance-warn jlink-modules guestjob-quote guest-launch sshkeys; do
    spec=${EXT_LABELS[$label]}
    title=${spec##*|}
