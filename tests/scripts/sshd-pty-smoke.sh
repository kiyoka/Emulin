...
if [ "$ready" != "1" ]; then
    echo "FAIL sshd-pty-smoke : sshd did not start listening within ${SSHD_WAIT}s"
    echo "--- sshd log tail ---"
    tail -20 "$SSHD_LOG"
...
OUT=$(timeout "$SSH_TIMEOUT" ssh -tt -p "$PORT" -i "$TKEYDIR/clientkey" \
    -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
    -o ConnectTimeout="$SSH_CONNECT_TIMEOUT" \
    -v \
    root@127.0.0.1 "tty; echo $EXPECTED; exit" 2>"$SSH_ERRLOG")
RC=$?
...
if [ "$RC" != "0" ]; then
    echo "FAIL sshd-pty-smoke : ssh client exit=$RC (ConnectTimeout=${SSH_CONNECT_TIMEOUT}s / timeout=${SSH_TIMEOUT}s)"
