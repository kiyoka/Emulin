...
# ★ issue #1018: env で変えられるようにする。**負のコントロールが取れない検査は
#   検査になっていない** (TIMEOUT=1 にして「timeout で殺された」と出るか試せる)。
#   負荷で伸びたときに上げられる意味もある (#1015 で ssh 軸が同じ形だった)。
TIMEOUT=${TIMEOUT:-120}  # wget HTTPS で TLS handshake + 読み込みに ~50 sec かかる
...
run_case() {
    ...
    if printf '%s' "$act" | grep -F -q -- "$pat"; then
        ...
        printf 'FAIL    real-coreutils-%s : guest was killed by timeout after %ss\n' "$name" "$TIMEOUT"
        ...
        if [ "$rc" != 0 ]; then
            printf 'FAIL    real-coreutils-%s (guest rc=%s)\n' "$name" "$rc"
        else
