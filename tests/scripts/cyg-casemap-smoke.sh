...
if [ "$GUEST_RC" != 0 ]; then
    if [ "$GUEST_RC" = 124 ] || [ "$GUEST_RC" = 137 ]; then
        echo "FAIL    cyg-casemap-guest-finished : guest was killed by timeout after ${CYG_TIMEOUT}s (実測 ${CYG_ELAPSED}s)"
        echo "        (負荷で伸びたなら CYG_TIMEOUT を上げる。伸びていないなら本体の停止を疑う)"
    else
        echo "FAIL    cyg-casemap-guest-finished : guest exited rc=$GUEST_RC (途中で死んだ)"
    fi
    ...
    echo "===== cyg-casemap smoke: PASS=0 FAIL=1 (guest が最後まで走らなかった) ====="
    exit 1
fi
