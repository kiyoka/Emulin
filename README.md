...
| **Guest file access** | Chooses which host folders the guest may see (the allowlist from issue #732). **Empty means no restriction** — the guest, and anything it runs, can read and write your files. Add one or more folders and everything else on the host is reported as "does not exist" (the guest's own root filesystem is always allowed). Takes effect for guests started from then on. The setting is saved in `%USERPROFILE%\.emulin\fs-allow.txt` and **also applies to guests you start from a command prompt**, not just from the launcher |
