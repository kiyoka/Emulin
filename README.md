...
1. **Java をインストール** (PATH に通す)
   - [Adoptium Temurin](https://adoptium.net/) の OpenJDK 21 LTS が推奨
   - インストール後、`Win+R` → `cmd` で:
     ```cmd
     java -version
     ```
     `openjdk version "21..."` のように表示されれば OK

2. **配布 zip をダウンロード**
   - [Releases](https://github.com/kiyoka/emulin/releases) から
     `emulin-dist-<version>.zip` をダウンロードして任意の場所に解凍
     (例: `C:\Tools\emulin\`)
...
5. **(オプション) 実機 Linux binary を動かす**
   git や openssl 等の動的リンクされた実機 binary を動かすには、Linux 上で
   `dist/build-sandbox.sh` を実行して sandbox を組み立て、生成物 (rootfs)
   を Windows へコピーします。Debian/Ubuntu (WSL や Linux VM) を介する形が
   実用的です。詳細は「実機 Linux binary を動かす」を参照。

> **JRE 同梱版** (Java 不要・解凍するだけで動作):
> - `emulin-jre-<ver>-windows.zip` — Windows 用 JRE 同梱 (busybox のみ、~22 MB)
> - `emulin-demo-<ver>-windows.zip` — Windows 用 JRE + 実機 binary (git/curl/openssl 等) 同梱 (~80 MB)
>
> Linux / macOS 用も同様に `-linux` / `-macos` suffix の zip が並びます。
> rootfs (Linux ELF binary 群) は全 platform 共通、JRE のみ host native です。
