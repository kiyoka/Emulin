...
> As of 0.8.4, prebuilt release zips are published **for Windows only**
...
2. **Download the distribution zip**
   Get `debian-emulin-0.8.4-windows-x64.zip` from
...
3. **Unzip anywhere**
   e.g. `C:\Tools\debian-emulin-0.8.4-windows\` (paths with Japanese characters
...
4. **Start the bash interactive shell**
   ...
   ```cmd
   cd C:\Tools\debian-emulin-0.8.4-windows
   emulin.bat
   ```
...
5. **Single-command mode / running real binaries**
   `debian-emulin-0.8.4-windows` bundles git / curl / openssl / python3, etc.,
...
`debian-emulin-0.8.4-windows-x64.zip` is built on a rootfs that is
