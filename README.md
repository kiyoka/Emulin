...
| **Open X terminal** | Opens the guest's `xterm` **on an X server running on Windows** (VcXsrv / XLaunch). Needs an X server on `127.0.0.1:6000` and `apt install -y xterm x11-apps` in the guest; the button checks both **before** it starts anything and tells you which one is missing (issue #1021) |
