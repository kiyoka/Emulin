...
- **D. IPv6 (AF_INET6) は部分対応** — client TCP のみ実装済み (socket /
  connect / getsockname / getpeername、PR #35、回帰 `sys_inet6_64`)。
  UDP v6 と server v6 (bind/listen/accept) は EAFNOSUPPORT のまま。
  getaddrinfo は AAAA を引いたら v6 path に乗り、失敗時 v4 fallback で
  clean に走る。WSL2 は v6 routable でないので実 v6 connect は host と同じく
  fail (clean に)。
