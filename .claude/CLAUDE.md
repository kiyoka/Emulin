...
- **回帰テスト 221 PASS / 0 FAIL** (Phase 28-3 で 211 → 221、bash 6 +
  git-clone 4)。`run-fast.sh` 27 秒、`run-network.sh` 3 分、フル
...
### B. 多サイトの HTTPS 動作確認
github は 28-3 の workaround 込みで動作。**他サイト (cloudflare、example.com、
google) は未テスト**。各サイト別 root CA を事前準備する仕組みが必要。

### C. file system / fork 系
**`git clone --hardlinks file://`** が hardlink 同一性検証で失敗。
`--no-hardlinks` 指定で動作。`Inode.get_uniq_no` を host と揃える調整が必要。
