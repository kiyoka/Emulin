...
- 回帰テスト 47 本 PASS (`sys_chmod64` 含む 3 件は WSL DrvFs 制約で SKIP)
...
6. **0x66 prefix 全面監査** — 既知バグの傾向 (Phase 11/14/20) からして
   imm を持つ命令 (0x69/0x6B IMUL imm、0x81 imm32-with-66、0xA9 TEST
   acc imm 等) で他にも漏れがある可能性。一通り `0x66 + imm` 系を
   見直すと将来の不可解バグを潰せる
