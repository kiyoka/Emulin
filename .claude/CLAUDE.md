...
- **git clone HTTPS の CA 認識が NG** (step 41): TLS handshake は完全成功し
  pack 受信まで完走するが、libcurl-gnutls が `/etc/ssl/certs/ca-certificates.crt`
  をうまく読めず `sslVerify=false` が必須。step 37 の pipe IPC deadlock は
  CLOEXEC fix で完全解消済み
