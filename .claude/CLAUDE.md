...
- シグナル基盤: SIGCHLD 自動配信、SA_RESTART で syscall 自動再開、
  ...
  sigaction.sa_mask + SA_NODEFER 完全対応
...
- **pthread の基礎動作**: pthread_create + worker + pthread_join + 共有
  memory が動作 (Phase 27 step 28、Thread64 + FutexManager + clone CLONE_VM
  対応)。複数 worker / mutex 競合等の安定性は今後の検証
- **回帰テスト 209 本 PASS / 0 FAIL / 0 SKIP**、xargs -P / wait -n で
