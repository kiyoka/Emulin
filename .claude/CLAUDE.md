...
- **wget HTTP/HTTPS、curl HTTP、wget DNS が動作**。wget HTTPS で iana
  (RSA cert + TLS 1.3) は確実に動作。**ECDSA cert は server FIN** (step 19-20)
...
- **gnutls CA load 完全 fix** (step 44): 真因は **MOVSX/MOVZX r16 (0x66
  prefix) の上位ビット破壊** + **PINSRW / 0F 3A 系の RIP-relative
  addressing で imm8 のサイズ未加算**。修正後、全 cert load 成功
  (`test_pem_load` で rc=0 確認)
- **git clone HTTPS は CA load 後の TLS handshake で ld.so symbol resolve
  バグ** (step 45 で原因特定): CA load 自体は完璧動作するが、TLS handshake
  中に libtasn1 が `free()` を呼ぶ際、ld.so がシンボル解決で **誤った DSO の
  load_bias** (libgnutls の base) を使い、GOT[free] = libgnutls+0xadd50 を
  書く。実際の libc の free symbol は同じ vaddr 0xadd50 にあり、libgnutls
  との偶然の同オフセットで間違いと気付かれず、その後 free 呼び出しで
  libgnutls の `gnutls_pkcs11_copy_x509_crt2+0x220` 内に飛んで segfault。
  ld.so の relocation 処理 (`elf_machine_rela`) が間違った link_map の
  l_addr を使用している模様。深掘りには ld.so 内部の追跡が必要
- **git clone HTTPS (GitHub) も完全動作** (step 41): `git -c http.sslVerify=false
  clone https://github.com/octocat/Hello-World.git` で完走。CA cert 認識は
  別問題で `sslVerify=false` 必須 (step 37 の pipe IPC deadlock は CLOEXEC
  fix で完全解消)
- **git clone (git:// および file://) が完全動作** (step 40 で完走):
  `git clone --no-hardlinks git://git.kernel.org/pub/scm/utils/dtc/dtc.git`
  で 346 obj 受信 + 38 deltas 解決 + 335 ファイル checkout 全て完走。
  step 39 (CLOEXEC) + step 40 (mmap guard gap + prlimit64 + wait4 specific pid)
  の 3 段階で達成
