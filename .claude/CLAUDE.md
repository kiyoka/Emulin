...
- **git clone HTTPS hang** (step 37): git → git-remote-https の pipe IPC
  deadlock (curl HTTPS の遅さとは別バグ)
- **git clone file:// は動作 (--no-hardlinks 必須)** (step 39 で fix):
  CLOEXEC + main thread sys_exit fix で `git clone --no-hardlinks file://...`
  が完走 (working tree も checkout)。ハードリンク無し前提なら local clone OK
- **git clone git:// (network) は post-pack で segfault** (step 39 で部分的に
  進歩): pack 受信 + delta 解決完走するが pthread worker exit 後 main が
  RIP=0x401525e9 (segment 範囲外) で segfault。原因不明だが RIP 値の上位ビット
  消失パターン → LEA 系の zero-extend バグ類似が疑われる (Phase 27 step 3 と同型)
