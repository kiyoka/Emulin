...
- **git clone HTTPS (cert verify あり) で libtasn1 free に bogus pointer**
  (step 47 で更に深堀): CA load + ld.so symbol 解決はすべて完璧動作。
  GOT[free] は libc free の正しいアドレス。
  crash の根本: libtasn1 が `asn1_der_decoding2` 内 `call free@plt` で
  `r14+0x50` (= ASN.1 node の `value` field) を free。期待値は NULL or
  malloc'd pointer or `&node->small_value` だが、emulator では
  **0x1440 (= libc malloc 内部の chunk size 値)** が読まれる。
  - calloc は struct を zero 初期化済み (確認済)
  - libtasn1 は struct の他 field (offset 0x10/0x44/0x48/0x70) を書くが
    offset 0x50 (value) は 一切書かない (watchpoint で確認)
  - libc malloc が 0x555555b7a010 (= struct + 0x50) に chunk header と
    して 0xc0 / 0x240 / 0x1380 / 0x1440 を書いている (chunk 分割の痕跡)
  - 同領域で free(0x555555b7a020) (struct + 0x60 内部) も呼ばれている
    → libtasn1 の struct chunk と他の chunk が overlapping している疑い
  - ld.so / glibc malloc の内部追跡が必要。深掘り作業
