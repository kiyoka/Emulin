...
- 動的リンク + 複雑な libc 機能が動作 — printf 各種 (NaN/Inf/bignum %g/%e/%a)、
  ...
  fork+pipe IPC、zlib、socketpair (片方向)、mmap of file、GCC ネスト関数
...
- **wget HTTPS で実 HTML 取得**: iana / kernel.org / python.org / github
...
- **Cloudflare/Google の TLS 1.3 ECDSA で server FIN** (Phase 27 step 19)
  — example.com / cloudflare.com / google.com が handshake 完了後に
  server から close される。crypto primitive (SHA384/AES-256-GCM/HMAC/AES-NI)
  単体は host と完全一致。ClientHello バイト長 (我々 400 vs host 308) が
  唯一の差分で、anti-DoS / TLS fingerprinting (JA3/JA4) で silently FIN
  されている疑い。iana / github / kernel / python は動くので回帰には影響なし
...
- **socketpair は片方向のみ** — Phase 25 で pipe 流用の片方向設計。真の
  双方向化は Fileinfo に read_pipe / write_pipe 二系統が必要 (将来課題)
...
- 小物: tgkill / MOVMSKPD / JRCXZ / PMAXUB / futex no-op / PINSRW /
  socketpair (片方向、pipe 流用) / mmap of file / clone3 → ENOSYS /
...
### step 19: Cloudflare/Google TLS 失敗 (調査のみ・修正なし)
- 動く: iana (RSA + AES_256_GCM + SHA384), github (ECDSA + AES_128_GCM
  + SHA256), kernel.org, python.org
- 動かない: example.com / cloudflare.com / google.com (いずれも ECDSA
  + AES_256_GCM + SHA384 の Cloudflare/GFE 系 CDN)
- 確認済み (= バグでない): SHA384 transcript hash (長さ 0〜7900 で host
  と完全一致)、AES-256-GCM ラウンドトリップ、HMAC-SHA384、AES-NI 7 命令、
  ECDSA + AES_256 の組み合わせ自体 (iana の RSA 経路で AES_256 動作)
- 唯一の差分: ClientHello バイト長 (我々 400 byte、cipher suite 75 件 vs
  host 308 byte、~30 件)。openssl-min.cnf で cipher 制限しても変わらず
- 推定原因: Cloudflare/GFE の anti-DoS / TLS fingerprinting (JA3/JA4)
  が特異な ClientHello を silently FIN
- 残された手筋: 自前 client で host 互換 ClientHello を作って投げる、
  OPENSSL_CONF / SSL_CIPHER_LIST 直接指定、ECDSA verify skip 検証、
  PCAP で host vs emulator 完全比較
...
5. シグナル更なる拡張: SIGALRM 自動配信 (alarm/setitimer)、sigprocmask、
   sa_mask 中ハンドラの再入抑止
6. dist の充実: jlink で JRE 同梱の self-contained zip、Win/Linux/macOS
   別の zip 出力
