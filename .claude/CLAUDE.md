...
# Phase 18: getdents64 の重複 "." 問題を修正

実施日: 2026-04-30 (継続)
作業ブランチ: `phase18/getdents-duplicate-dot`

## バグ

`ls -a /etc` の出力に `.` が 2 回現れる長年の既存バグ:
...
`FileAccess.file_list` の `.` `..` 補完ロジックが誤っていた:
...
Java の `File.list()` は `.` `..` を含まない仕様なので、ほぼ常に
`slide=3` 経路に入って `.` が 2 回追加されていた。本来は `.` と `..`
を各 1 回ずつ追加すべき。
...
## 新規回帰テスト: `sys_getdents64`

`/etc` を getdents64 で列挙し、`.` `..` `emulin.cnf` が **各 1 回ずつ**
返ってくることを直接検証する単体テスト。

`tests/binaries/src/sys_getdents64.c`:
```c
volatile long n = sys_getdents64(fd, buf, sizeof(buf));
// 各エントリを byte 単位でパースしてカウント
// 期待: dot=1 dotdot=1 emulin=1 other=0
```

期待値: `dot=1 dotdot=1 emulin=1 other=0`

## 副次修正

`tests/expected/myls64.stdout` に古いバグの `.` 重複が含まれていたため
正しい順序 (".", "..", "emulin.cnf") に修正。

## 回帰テスト

**46 PASS / 0 FAIL / 1 SKIP** (新規 `sys_getdents64` で 1 増、既存維持)。
