...
- **実機 GNU coreutils 9 種類が動作** — ls / cat / wc / echo / true /
  false / dirname / basename / uname (動的リンク + libc + libselinux +
  libpcre2 + libacl) (Phase 27 step 1-2)
...
- **回帰テスト 151 本 PASS / 0 FAIL / 0 SKIP** — sandbox を Linux 側 /tmp に
...
- `tests/scripts/real-coreutils.sh` — 実機 GNU coreutils (ls / cat / wc /
  echo / true / false / dirname / basename / uname) を sandbox にコピー
  して 11 ケースを Emulin で回す回帰 (Phase 27)。host に対象バイナリ /
  ld-linux / libc.so.6 が無ければ SKIP。
...
未対応 (Phase 27 で発掘した残課題):
  - `/bin/sed` — load8 segfault (sed 内部の malloc/buffer 経路で out-of-bounds)
  - `/bin/grep` — 無限ループ (poll の ready 即返しが busy loop を誘発か)
  これらは Phase 27 step 3+ で取り組む。

## Phase 27 の到達点 (進行中)

- 回帰 **151 PASS / 0 FAIL / 0 SKIP** (Phase 26 の 140 + real-coreutils 11 本)
- 実機バイナリ 9 種類 (動的リンク GNU coreutils) が動作
