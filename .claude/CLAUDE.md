...
- **git clone HTTPS の libcurl-gnutls 側 CA load NG** (step 42 で部分前進、
  step 43 で原因絞り込み): step 42 で UTF-8 ファイル名問題を修正、全 295
  cert が open 成功。step 43 で `EMU_GNUTLS_DEBUG_LEVEL=9` を使い真因絞り込み:
  - **n=1 cert ファイル**: gnutls が "found 1 certificates" で正常 load
  - **n=2 以上**: 「Could not find '-----BEGIN X509 CERTIFICATE'」で fail
    (Base64 unexpected header error)
  - 静的 C プログラム (fopen + fread + memmem) では emulator 上でも全 146
    BEGIN marker を正常検出 → file I/O / memmem は emulator OK
  - gnutls の `_gnutls_fbase64_decode` 内部の独自 PEM parser に固有の
    挙動 (詳細は要追跡)。curl (libssl) は同じ ca-certificates.crt で
    完璧動作なので gnutls API 固有
