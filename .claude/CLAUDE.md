...
## 現状サマリ (Phase 34-A3 step 22 / SIB + PUSH/POP/MOVZX 完了時点)
...
avg block size の最終 (sha256sum 5MB):
  step 14: 1.82
  step 18: 1.83 (hot threshold で cold block 除外)
  step 19: 2.19 (SIB)
  step 20: 2.27 (imm32)
  step 21: 2.57 (PUSH/POP/LEAVE)
  step 22: 2.57 (MOVZX/MOVSX register form は hot path 出現なし)

性能 (5MB sha256sum, system load によって ±3s variance):
  no JIT:  ~15-17s
  JIT:     ~15-19s  (基本 neutral、system load 高で +20% 程度)

→ step 14 までの -9% speedup は失われた。理由は opcode 増加で生成
  bytecode 肥大化 → JVM JIT inlining 限界 → 1 block の execute() が
  C2 で fully inline されず INVOKEINTERFACE megamorphic dispatch コスト
  が増えた
