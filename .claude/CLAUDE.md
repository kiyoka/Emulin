...
- **動的リンク + 複雑な libc 機能が動作** — printf 各種フォーマット
  ...
  IPC、zlib (compress/uncompress) (Phase 25)
...
- **回帰テスト 135 本 PASS / 0 FAIL / 0 SKIP** — sandbox を Linux 側 /tmp に
...
- 動的リンクで hello より複雑な libc 機能 (printf 全フォーマット /
  malloc / setjmp / strftime / sscanf / qsort / regex / dlopen /
  wchar / C++ STL + 例外 / fork+pipe IPC / zlib 圧縮 等) が動作
- bignum 系の致命バグ (PF 未追跡、ADC/SBB フラグ未更新) を解消
- 動的リンク回帰固定 5 本 (hello / printf / regex / cpp / zlib 各 _dyn64)
- 回帰 **135 PASS / 0 FAIL / 0 SKIP** (旧 130 + 動的リンク 5 本)
- 未対応のまま:
    - **socketpair (#53), AF_INET socket** — pipe ベースの簡易実装が
      可能だが本物の双方向通信には fd ごとに独立した read/write
      バッファが要る (FileAccess 拡張が必要、工数大)
    - **mmap of regular file** — alloc_and_map は動くが、glibc の
      DSO_FALLBACK 等で MAP_FIXED + 細かい配置を要求されると segfault
    - **pthread (clone3 #435 + 真の futex)**
    - **GCC nested function の実行可能スタック**
...
2. **`/bin/ls`, `/bin/bash`, `gcc` 等の本物のバイナリを動かす**
   (#1 完了が前提)
3. **pthread 対応** — clone3 (#435) + futex (#202) + TLS 拡張。
   工数大。
