...
- **★ git clone HTTPS (cert verify あり) で github 完全動作 (step 59)**:
  step 58 で libtasn1 segfault 解消、step 59 で TLS handshake error も解消。
  3/3 runs で README まで clone (14 秒)。
  - 必要な sandbox 設定:
    1. locale files を copy (`cp -r /usr/lib/locale/C.utf8 ...` 等)
    2. `/etc/ssl/certs` から test cert files (c1.pem 等) を削除
    3. `/etc/gitconfig` に `sslCAInfo = <single root cert>` と
       `sslCAPath = ` (empty) を設定
  - 内部的には emulator が遅くて 440 cert load に 83 秒かかり server idle
    timeout で FIN される。1 cert に絞ると 14 秒で完走
  
- **(残課題, 旧) git clone HTTPS で libtasn1 内 segfault — step 58 で解決済み**
  (step 48 で更に絞り込み):
