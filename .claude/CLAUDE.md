...
## 現状サマリ (Phase 27 完了 + step 60-64 最適化、Phase 28 開始)
...
# Phase 28: ディストリビューション整備
*(進行中)*

## Phase 28 の目標 (検討中)

現状の dist:
- `dist/build-dist.sh` → `emulin-dist-<ver>.zip` (約 1.7 MB)
- 中身: fat jar + busybox + ランチャ (.sh / .bat) + 空 rootfs

検討候補:
1. **sandbox 整備**: 実機 binary 動作の前提 (locale files / ld.so.cache /
   /etc/gitconfig 等) を sandbox 構築スクリプトとして提供
2. **jlink で JRE 同梱**: java インストール不要にする (Linux/macOS/Windows 別 zip)
3. **dist 内容物の充実**: openssl / git / curl のサンプル sandbox を同梱
4. **CI / リリース自動化**: GitHub Actions でビルド + zip 配布
5. **README 充実**: 実機 binary 実行方法、sandbox 作成方法
