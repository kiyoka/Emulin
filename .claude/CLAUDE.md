...
3. dist の充実: jlink で JRE 同梱の self-contained zip、Win/Linux/macOS
   別の zip 出力
4. ITIMER_VIRTUAL/PROF (CPU 時間ベースのタイマ) — 現在は no-op、
   実用上は ITIMER_REAL で代用可能なケースが多いが厳密でない
5. sa_mask 拡張: ハンドラ進入時に sigaction の sa_mask フィールド
   を読んで複数 signal を mask する (現在は配信中の signal 自身のみ)
