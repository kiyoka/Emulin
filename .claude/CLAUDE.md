...
- **Windows native で git clone → rm -rf 完走 + Ctrl-C で emulin が
  終了しない** (Phase 33 系)
- **Windows native で emacs-nox 29.3 完全対話動作 (C-x C-c / C-x C-f /
  resize)** (issue #3-#5: amd64_poll wait-loop + pselect6 bitmap
  write-back + FIONREAD TTY + JLine ISIG=off の 5 連 fix)
...
## issue #3-#5: emacs-nox 29.3 が Windows native で完全対話動作

INCLUDE_EMACS=1 で build した Windows demo zip で emacs-nox を実行すると、
splash 画面までは表示されるが C-x C-c / C-x C-f / 文字入力すら一切反応
しない症状。原因は単一ではなく **5 件の bug の連鎖** だった。

切り分けに使った診断 env: `EMULIN_DEBUG_TTY=1` を立てると JLineConsole +
pselect6 + poll + read 経路に密度高めの DBG_* ログが出る (`DBG_PSELECT`
`DBG_PSELECT_RET ready=N rfds=0x...` `DBG_AVAIL true/false`
`DBG_READ fd=N name='...' isSTD=...` `DBG_RD got=0x...`
`DBG_SYS_AFTER_PSELECT #<sysno> ...`)。

### #3-#5 (a) amd64_poll が 1 回 scan で即 return → wait-loop に refactor
旧実装は fds を 1 周なめて即 `return ready` だった。`poll(STDIN, -1)`
(無限 blocking) で TTY raw mode + Available()=false なら 0 が即返り、
caller が busy loop。pselect6 と同じ deadline+sleep 10ms の wait-loop に
拡張。socket peek の per-iter wait_ms も 10ms に短縮 (total timeout は
outer loop が制御)。glibc 2.34+ で ppoll(#271) も使うので、未実装だった
ppoll を poll の wrapper として新規追加 (timespec → ms 変換)。

### #3-#5 (b) pselect6 の max_iter=200 cap を撤去
Phase 30 follow-up8 で「finite timeout は 200 iter = 2 秒で打ち切り」と
していたが、emacs は `timeout = 100000000 ms` (≒27 時間 = 実質 infinite)
で pselect6 を呼ぶため、2 秒で 0 返却 → emacs main loop に戻る → 再
pselect6 → 2 秒... を繰り返し key 取りこぼし。deadline_ms チェックが
あれば iter cap は不要 (= `max_iter = Integer.MAX_VALUE`)。

### #3-#5 (c) pselect6 が結果 bitmap を write-back する
Linux 仕様: `pselect` 戻り後 `readfds` は **ready な fd のみ bit を残し他は clear**。
旧実装は input bitmap を全く touch せず、ready=N を返すだけ。emacs は
`FD_ISSET(fd, &Available)` で「どの fd が ready か」を判定するため、未
clear だと「全 fd ready」と誤解。ready 計算と並行で `new_rfds[]` /
`new_wfds[]` を build し、最後に `mem.store64` で書き戻す。exceptfds は
常時 0 clear。

### #3-#5 (d) FIONREAD の TTY 対応 ← 決定打 ★
emacs の input loop は **(1) pselect で readable 待機 → (2)
`ioctl(fd, FIONREAD, &n)` で実 byte 数確認 → (3) n >= 1 なら read** と
いう三段構え。我々の FIONREAD 実装は TTY fd で常に 0 を返していたため、
emacs は「select が ready と言ったが FIONREAD は 0 = select の誤判定」
として read を skip。pselect が ready=1 を返し続けるのに emacs が
read syscall を一切発行しない (DBG_RD が 0 件) 真因。

`DBG_SYS_AFTER_PSELECT` で emacs の ready=1 後の syscall を見たら
**`#16 ioctl a2=0x541b` (FIONREAD) が fd 3 に対して必ず呼ばれて**
いることで確定。修正: TTY fd の FIONREAD で `console.Available()` が
true なら最低 1 を返す (JLine の NonBlockingReader は ready() しか
公開していないが、emacs は >= 1 で OK)。

### #3-#5 (e) enterRawMode で ISIG=off を明示 ← Ctrl-C 専用 fix ★
C-x C-f / C-g は効くのに **C-x C-c の C-c だけ届かない** 症状。
JLine の default `enterRawMode()` は ICANON/ECHO/IEXTEN を off にする
が ISIG flag を touch しない。ISIG=on のままだと
`AbstractWindowsTerminal.processInputChar` が VINTR (0x03) / VQUIT
(0x1C) / VSUSP (0x1A) を signal に変換し `terminal.handle(INT)` を
発火させて **byte を reader に届けない**。emacs は C-x prefix で待ち
続け C-c が来ないので終了しない。

修正: `enterRawMode()` 直後に `attrs.setLocalFlag(LocalFlag.ISIG, false)`
+ `terminal.setAttributes(attrs)` を呼ぶ。savedAttrs は不変なので
exitRawMode 時に自動復帰。これで Ctrl-C も byte 0x03 として emacs/vim
/bash readline に直接届く。Phase 33-17 → issue #3-#3 で整備した
「raw mode 中は emulator 側で SIGINT 配信 skip」と完全に整合。

C-x (0x18), C-g (0x07), C-f (0x06) 等は VINTR/VQUIT/VSUSP/VSTATUS と
重ならないので元から素通り。C-c (0x03 = VINTR default) だけが捕捉
されていた、というのが「prefix は動くのに C-c だけ動かない」謎の正体。

### 関連 cleanup
- `Process.run` の epilogue で `cpu.cache_expire()` / `syscall.all_file_close()`
  に null-guard を追加。exec 失敗で cpu が初期化されない経路で NPE が
  出ていた (`/usr/bin/vim` が rootfs に無い等のとき)
- `Console.check_and_send_int` は既に Phase 33-17 / #3-#3 で raw mode
  時は SIGINT 配信を skip するようになっていたが、(e) の ISIG=off と
  併せて初めて完全動作

### 検証
- 回帰 `tests/scripts/run-fast.sh`: 146 PASS / 0 FAIL
- Windows demo (emacs-nox 含む) on Windows 11 Home + Windows Terminal:
  - splash 表示 ✓
  - C-x C-f → minibuffer "Find file:" 表示 ✓
  - C-x C-c → emacs 正常終了 ✓
  - 文字入力 ✓
  - **window resize → 新サイズで再描画** ✓ (SIGWINCH 配信は既に整備済)
- Windows demo cmd.exe / Windows Terminal の差: 当初 Ctrl-X / Ctrl-C
  が両方とも届かないと誤判断したが、実は (d) FIONREAD と (e) ISIG が
  両方発火していたためで、conhost 起因ではなかった
