...
## Phase 27 step 19: Cloudflare/Google TLS 失敗の追跡 (調査のみ・修正なし)

step 18 で残課題とした「example.com / cloudflare / google で handshake
後に server が FIN する」現象を追跡。crypto primitive 単体は全て正しく
動くことを確認したが、server 側の TLS fingerprinting が原因の可能性
が高く、emulator 側の修正で対処できる確証が得られなかったため
**保留**。

### 動く / 動かないの線引き (4 動 / 3 不動)

  - 動く: www.iana.org (RSA + AES_256_GCM + SHA384), github.com
    (ECDSA + AES_128_GCM + SHA256), www.kernel.org, www.python.org
  - 動かない: example.com / www.cloudflare.com / www.google.com
    (いずれも ECDSA + AES_256_GCM + SHA384、Cloudflare/GFE 系 CDN)

### 確認済み (= バグでない)
  - SHA384 transcript hash: 長さ 0〜7900 で host openssl と完全一致
  - AES-256-GCM ラウンドトリップ: 各種長で host と一致
  - HMAC-SHA384: host と一致
  - aesni_static64 7 命令: host AES-NI と完全一致
  - ECDSA + AES_256 の組み合わせ自体は iana の RSA 経路で AES_256
    が動く ⇒ AES key derivation は OK

### 唯一の差分 (推定原因)
  - ClientHello バイト長: emulator 400 byte vs host 308 byte
  - 差分の大半は cipher suite list (我々は 75 件、host は ~30 件)
  - openssl-min.cnf で cipher を制限しても変わらず (system_default_sect
    が wget 経由で適用されない可能性)
  - Cloudflare/GFE は anti-DoS / TLS fingerprinting (JA3/JA4) が活発
    なので、特異な ClientHello を silently FIN している疑い

### 残された手筋 (step 20+ で再開する場合)
  1. 自前 client (TLS 1.3 socket を直接組み立て) で host 互換 ClientHello
     を作って Cloudflare に投げ、emulator で再現できるか確認
  2. wget 起動時の OPENSSL_CONF / SSL_CIPHER_LIST を直接指定して
     cipher suite list を 5-6 個に絞る
  3. ECDSA verify を skip する (--no-check-certificate でも我々の
     OpenSSL は cert verify している可能性)
  4. PCAP を取って host vs emulator の ClientHello を完全比較

回帰追加なし、コード変更なし。CLAUDE.md だけ更新して step 19 を
クローズ (実装上の進展ゼロを明示するため)。
