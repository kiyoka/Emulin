...
- **Phase 29-tty2**: `wait4(WNOHANG)` で yield + 1ms sleep を挟む。tight
  ...
  E79 を起こしていた fork/exec race を緩和。完全解決には fork CoW
  実装が必要 (vim 対話モードはまだ E79 多発、ex mode は OK)
...
- **★ wait4 WNOHANG で tight polling は emulator killer** (Phase 29-tty2)
  — 即座に return すると Java thread scheduler が child を実行する
  機会がなく、parent が「子は永遠に動かない」と判定して fail。
  `Thread.yield() + Thread.sleep(1ms)` を挟む。fork CoW なしの emulator
  では特に重要 (child の起動が遅いため)
