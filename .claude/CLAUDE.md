...
- シグナル基盤: SIGCHLD 自動配信、SA_RESTART で syscall 自動再開、
  ...
  sigaction.sa_mask + SA_NODEFER 完全対応、**per-thread signal mask + tgkill
  thread-targeted pending** (pthread_sigmask / pthread_kill 完全互換、step 34-35)
...
### step 35: thread-targeted pending signal (tgkill/pthread_kill 真対応)
step 34 で per-thread mask は対応したが、pending signal 自体は process
全体共有だったため、`tgkill` で特定 thread に送っても別 thread (unmasked)
が先着で消費する問題があった。step 35 で per-thread pending tracking 追加。

修正:
- `Signal` クラスに `ConcurrentHashMap<Integer tid, int[] pending>` 追加。
  key = `Thread64.tid` または `process.pid` (main thread)
- `recv_to_thread(target_tid, sig)`: 特定 thread の pending に enqueue
- `psig()` の優先順位: own thread の pending → process-wide pending
- `signal_cancel`: own thread の pending を先に消費
- `amd64_tgkill (#234)`: 旧実装は kill にフォールバック (= process-wide)
  だったが、`process.recv_to_thread(tid, sig)` で specific thread に送る

回帰拡張: `pthread_sigmask_dyn64` に `parent_handler_fired` check 追加。
- 親が SIGUSR1 を block
- 子から `pthread_kill(parent, SIGUSR1)` → parent の pending に入る (子は受け取らない)
- 子の handler が呼ばれた後 (child だけ unblock + kill self)
- 親が unblock → pending の SIGUSR1 が parent の handler で配信される

step 34 の design では「子が parent 宛て signal を消費してしまう」バグが
あったが、step 35 の per-thread pending で完全解決。

回帰 211 PASS / 0 FAIL / 0 SKIP のまま (5 連続安定、pthread_mutex も 5/5)。

### step 36: curl HTTPS 失敗の追跡 (調査のみ・修正なし)
curl HTTPS は依然 timeout する原因を `-v` verbose で追跡。

判明したこと:
- **TLS handshake 自体は完全に成功**:
  - ClientHello 送信
  - ServerHello + EncryptedExtensions + Cert + CertVerify + ServerFinished 受信
  - ChangeCipherSpec + ClientFinished 送信 (= TLS 1.3 handshake 完了)
- 直後に `OpenSSL SSL_connect: Broken pipe` で server が FIN
- step 19-20 の Cloudflare ECDSA TLS 1.3 と同じパターン (handshake は
  通るが server が直後に切る)
- iana.org に対して wget HTTPS は 10s で成功するのに、curl HTTPS は
  80s+ かかる (curl の load + libcurl 経由の OpenSSL は wget より重い)
- pthread DNS は無関係 (`--resolve` で bypass しても同じ)

仮説: emulator の TLS handshake が 80s かかる → server の idle timeout
(~60s 程度) を超える → server が FIN → curl の SSL_connect が Broken pipe

検証: wget HTTPS は 10s で server の timeout 内に handshake 完了 → 成功。
curl は libcurl + 多数の library 経由で同 handshake が 80s → 失敗。

Pthread の overhead や thread switch race ではなく、純粋な curl/libcurl
の processing time が遅すぎることが原因。

未対応: curl HTTPS を確実に動かすには更に大幅な emulator 高速化が必要。
TLS 1.2 / 1.3 forced や `--resolve` で bypass しても同じ。fundamentally
curl 経由の HTTPS は emulator 上で太刀打ちできない。
- 代替: wget HTTPS は実用的に動く (10s)
- 別解: openssl s_client + nc 的なシンプルパスは試す価値あり

回帰追加なし、コード変更なし。

### step 37: git clone HTTPS の追跡 (調査のみ・修正なし)
git clone https://github.com/octocat/Hello-World.git を試行 → 3 分以上待っても
"Cloning into..." の後で停止。

調査: 
- emulator は user CPU 2.7s しか使っていない (≠ CPU-bound)
- syscall trace で最後は `read(fd=0, buf, 4096)` で blocked
- git は fork + execve で `git-remote-https` を spawn
- git-remote-https は stdin (pipe from git 親) を read 待ち
- git 親が pipe に何も書いていない (or 書いた内容が child に届いてない)

git --version / git init / git status (local) は 1〜2 秒で動作。
HTTPS-fetch する subprocess (git-remote-https) との pipe IPC が壊れている
模様。execve 越しに pipe fd を継承する経路に問題がある可能性。

curl HTTPS は emulator のスピード問題 (step 36)、git clone HTTPS は
inter-process pipe deadlock — **異なるバグ**。

回避策:
- ssh 経由の git clone は試していない (sshd 鍵生成等が emulator 上で
  時間がかかるかも)
- file:// 経由なら local pipe で動く可能性あり
