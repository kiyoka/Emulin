...
- 回帰テスト 79 本 PASS (`sys_chmod64` 含む 3 件は WSL DrvFs 制約で SKIP)
...
6. **dist の充実**: jlink で JRE 同梱の self-contained zip を作る
