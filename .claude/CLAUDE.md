...
- **回帰テスト 210 本 PASS / 0 FAIL / 0 SKIP**、xargs -P / wait -n で
  並列化、5 連続安定。`run-fast.sh` (real-* / dist 抜き) で 27 秒、
  `run-network.sh` (network のみ filter) で 3 分、フル `run-all.sh` 約 1m44s
  (step 30-33 で baseline 3m51s から **55% 短縮**)。
