...
- **git clone (git:// & file://) は完走しない** (step 38): `synchronized`
  撤去で git:// は **pack 受信 + delta 解決まで完走** (346 obj / 274 KiB / 38
  deltas done) するが、post-pack subprocess で segfault → refs 未書き込み /
  working tree 未 checkout。file:// (local) は git-upload-pack fork 直後に
  silent fail。pack は正しく届いているので転送経路は OK、subprocess 制御に
  別バグ
