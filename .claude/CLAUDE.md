...
- **git clone HTTPS の libcurl-gnutls 側 CA load NG** (step 42 で部分前進):
  step 42 で UTF-8 ファイル名 (例: NetLock の Hungarian の `Főtanúsítvány.pem`)
  が getdents64/loadString で `Q` に化けてた件を修正。全 295 cert が open
  成功するようになったが、それでも gnutls が `CAfile: none` と判定して失敗。
  curl (libssl 経由) は CA load も TLS handshake も完璧に動作 (Client Finished
  まで) → 失敗は別系統の gnutls API 経路
