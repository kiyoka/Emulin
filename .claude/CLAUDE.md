...
- **git clone HTTPS (cert verify あり) で libtasn1 segfault** (step 46 で
  真因特定): CA load + ld.so symbol 解決はすべて完璧動作。GOT[free] は libc
  free の正しいアドレス。crash の真因は libtasn1 の構造体 field (offset 0x50)
  が初期化されず stale malloc metadata `0x1440` を pointer として読み、
  `free(0x1440)` で chunk metadata access (`mov -0x8(%rdi),%rax`) が
  unmapped 領域にアクセスして segfault。emulator の memory init 系バグ
  または特定 CPU 命令の mis-emulation の疑い。深掘り必要。
