...
- **★ git clone HTTPS (cert verify あり) の libtasn1 segfault は step 58 で
  完全解決！** 真因は sandbox に locale files が無いため glibc が異なる
  malloc pattern を取って chunk overlap が起きていた。`/usr/lib/locale/C.utf8`
  などをコピーすると 3/3 runs で segfault が出なくなる。
  進化先のエラーは ECDSA TLS 1.3 (step 19-20 既知課題) で別系統。
