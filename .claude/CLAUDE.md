...
- **回帰テスト 193 本 PASS / 0 FAIL / 0 SKIP** — sandbox を Linux 側 /tmp に
...
- `tests/binaries/src/aesni_static64.c` — AES-NI / PCLMULQDQ 命令の
  回帰 (Phase 27 step 9)。AESENC / AESENCLAST / AESDEC / AESDECLAST /
  AESIMC / AESKEYGENASSIST / PCLMULQDQ の 7 命令を FIPS-197 test
  vector で検証。host (実 CPU の AES-NI) との完全一致を確認する。
...
## Phase 27 step 7: sys_rename / sys_mount / sys_pipe の (int) キャストを修正

step 6 で残った git status の load8 segfault を追跡して、致命的なバグを
発見・修正:

### 真因: Syscall.java の (int) キャスト

`Syscall.sys_rename` がアドレス引数を 32-bit に切り詰めていた:
```java
...
これは Phase 26 step 3 の sys_brk と同じ性質のバグ。i386 時代の名残で、
amd64 ではアドレスが 64-bit になっても long のまま渡すべきところを
int にキャストしていた。

具体症状:
  - body PIE が `0x555555554000` 起点でロードされ、
  - git の SHA1 一時ファイル名が `0x555555595e260` に置かれていた
  - rename(0x555555595e260, ...) → `(int)` で `0x5595e260` に化け
  - mem.loadString(0x5595e260) が unmapped 領域で segfault

Syscall.java を grep で監査して **3 箇所同種のバグを一括修正**:
  - `sys_rename` の name_from / name_to 引数 (上記)
  - `sys_mount` の devname / dirname 引数
  - `sys_pipe` の int[2] 出力配列ポインタ

これらは i386 経路では問題なかった (32-bit pointer なので) が、
amd64 経路で 64-bit pointer が来ると壊れていた。Phase 9 (Syscall.java
シグネチャ long 化) では「シグネチャだけ」直したので、内部の int
キャストの残党がここまで残っていた。

### 動作確認 (修正後)
  - `git -C /tmp/myrepo status -s` → ` M test.txt` (修正済みファイル表示)
  - `git --no-pager log --oneline` → `1a54fdc (HEAD -> main) initial`
  - 既存の coreutils も全部動作 (sys_pipe を使うものは多い)

回帰固定: real-coreutils.sh に git-status / git-log を追加 (40 ケース)。

## Phase 27 step 8: AES の追跡 (中間結果) + bash スクリプティング拡張

step 7 の流れで openssl rand の AES 失敗を追跡し、原因の場所までは
特定したが、深追いはしない方針に。代わりに bash の実用機能を拡張。
...
### 追加 syscall stub (Phase 27 step 8 までの差分)
  ...
  - `#439 faccessat2` → sys_access で代用 (bash heredoc で必要)

### 動作確認した bash スクリプティング
  - `[ -f /tmp/sample.txt ]` → if/elif/case
  - `for i in 1 2 3` ループ / `while [ $i -lt 3 ]` ループ
  - 配列 `a=(x y z); echo ${a[1]}`
  - リダイレクト `>` / コマンド置換 `$(cmd)`
  - `gcc --version` (GCC 13.3.0)

### 未対応のまま残しているもの
  - bash の heredoc (`<<EOF`) / process substitution (`<(...)`)
  - openssl rand / genrsa / enc -aes (AES key 経路)
  - gcc compile (内部 fork が深い)
  - curl --version (libkrb5 symbol resolution)

## Phase 27 step 9: AES-NI / PCLMULQDQ 命令を実装

OpenSSL の openssl rand 失敗を回避するため AES-NI 命令を実装。
3-byte opcode escape (66 0F 38 / 66 0F 3A) のサポートを追加し、
以下の命令を実装:

  - `66 0F 38 DC` AESENC      — AES 1 ラウンド暗号化
  - `66 0F 38 DD` AESENCLAST  — AES 最終ラウンド暗号化
  - `66 0F 38 DE` AESDEC      — AES 1 ラウンド復号
  - `66 0F 38 DF` AESDECLAST  — AES 最終ラウンド復号
  - `66 0F 38 DB` AESIMC      — InvMixColumns (decrypt key schedule 用)
  - `66 0F 3A DF ib` AESKEYGENASSIST — key expansion 補助
  - `66 0F 3A 44 ib` PCLMULQDQ      — carry-less 乗算 (GHASH 用)

実装は FIPS-197 準拠の S-box / 逆 S-box テーブル + GF(2^8) 乗算
helper を Cpu64.java に追加。

CPUID 更新: leaf 1 ECX に AES-NI(bit 25) と PCLMUL(bit 1) を立てて
0x02000003 に変更。

新規回帰: `aesni_static64.c` で 7 種類すべての出力を host との
完全一致で検証 (FIPS-197 test vector ベース)。Native と完全一致。

### openssl rand への影響

OpenSSL は AES-NI を使うようになり経路が `cipher_hw_aesni_initkey`
に切り替わったが、**同じ keylen=256 の問題**で fail。AES 命令自体
は問題なく動作している (aesni_static64 で完全検証) ので、AES の
keylen=256 問題は emulator の AES 実装ではなく、もっと上流
(OpenSSL の get_params 経路で kbits が 256→2048 に化ける) にある。
...
+ libcrypto + libgssapi_krb5 + libkrb5 等の動的リンク連鎖が必要。
試したところ libkrb5 の relocation 処理で
`undefined symbol: error_message` エラーで起動しないことが判明。

### 真因: Inode.get_uniq_no のハッシュ衝突

ld.so は `(st_dev, st_ino)` ペアでロード済みライブラリを識別し、
同じ値を持つファイルは「重複ロード」と判断して silently skip する。
emulator の `Inode.get_uniq_no(path)` が「char + index 合計」という
弱いハッシュで st_ino を生成していて、パス長が同じくらいの 2 ライブラリ
で衝突が起きていた:

  - `/lib/libcom_err.so.2` → 2017
  - `/lib/libhogweed.so.6` → 2017  ← **衝突!**

libhogweed が先に読まれていたので、libcom_err は ld.so に「これは
libhogweed と同じ ino だから既に loaded」と誤認識され、mmap せずに
即座に close されていた。LD_DEBUG=files でも `generating link map` の
ログが出ない (失敗の痕跡が残らない) ため発見が難航した。

### 修正

`get_uniq_no` を `pathname.hashCode()` (Java の 31-進多項式ハッシュ)
に置き換え。実用上ユニークで衝突確率が極めて低い。

### 動作確認

  - `curl --version` → curl 8.5.0 + libcurl/8.5.0 + OpenSSL/3.0.13 +
    多数の Protocol (https / ftps / 等) + Features (TLS-SRP / SSL / 等)
    が表示される
  - 実際の TLS 接続は AF_INET socket がまだ未対応のため未テスト

回帰固定: real-coreutils.sh に curl --version を 2 ケース追加
(timeout は 30→60 に拡張)。

### 副次対応 (Phase 27 step 10 までの差分)

- `Memory.alloc` の MAP_FIXED で `mark_address = adrs` していた箇所を
  「mark_address は前進のみ」に変更。低位 MAP_FIXED で mark_address が
  後退すると次の mmap(0) で既存領域と衝突する潜在問題を予防。
- `Kernel.boot` で env passthrough を `EMU_*` プレフィックスでも受け取る
  ように拡張。`EMU_LD_PRELOAD=...` 等で emulated process だけに env を
  渡せる (host JVM は影響を受けない)。
...
- 古い致命バグ 3 件を解消:
  ...
  - **sys_rename / sys_mount / sys_pipe の (int) アドレス切り詰め**
    (Phase 27 step 7) — git status で初発覚
  - **Inode.get_uniq_no のハッシュ衝突** (Phase 27 step 10) —
    libcom_err が libhogweed と st_ino 衝突して ld.so に silently skip
    されていた事故 (curl --version 失敗の真因)
...
- **既存 syscall ハンドラに残る int 切り詰め** (Phase 26 step 3 / Phase 27 step 7) —
  Phase 9 の long 化以後も、個別 syscall の中身に「ローカル変数が int」
  「引数や戻り値を `(long)bx & 0xFFFFFFFFL` でマスク」のパターンが残る
  ことがある。
    - sys_brk (Phase 26): 3 重に切り詰めて 0x555555559000 → 0x55559000
    - sys_rename / sys_mount / sys_pipe (Phase 27): name pointer を
      `int _p = (int)bx` で受けて `mem.loadString(_p)` していた。
      git status の rename(64-bit ptr, ...) で 0x55555595e260 →
      0x5595e260 に化けて segfault。
  低位アドレスでは見えず、PIE / 高位アドレスを使い始めると初めて
  顕在化する。**新しい syscall を追加するときはローカル変数 / 戻り値も
  全部 `long` で書く**こと。`grep -nE "= \(int\)(bx|cx|dx|si|di)"` で
  Syscall.java を監査して、アドレス引数 (path / buf 等) が int で
  受けられていないか確認するのが定番。
...
- **st_ino のハッシュは衝突に強くする** (Phase 27 step 10) — ld.so は
  `(st_dev, st_ino)` の対で「同一ファイル」を判定し、同じ ino のファイル
  を二重ロードしない。`Inode.get_uniq_no` の弱いハッシュ (char+index
  合計) で `/lib/libcom_err.so.2` と `/lib/libhogweed.so.6` が衝突して
  libcom_err が silently skip され、curl が `error_message` 未解決で
  起動しないバグを引き起こした (LD_DEBUG にも痕跡が残らない厄介な
  サイレント失敗)。**`String.hashCode()` のような衝突確率の低いハッシュ
  に統一すること**。低位下位 32-bit を ino にする時はマスク後の衝突に
  注意。
- **MAP_FIXED 後の mark_address は前進のみ** (Phase 27 step 11) —
  Memory.alloc が「addr != 0 のとき mark_address = addr」とすると、
  低位 MAP_FIXED 後に mark_address が後退して、次の mmap(0) で既存
  領域を踏む。`if( end > mark_address ) mark_address = end;` のように
  単調前進にする。
- **Java Socket と背景スレッド読み出しのレース** (Phase 27 step 11) —
  EmuSocket.connect() は SubProcess を spawn して socket を読み続ける
  i386 設計。amd64 で同じ Java InputStream を直接 Read すると、ヘッダ
  途中でデータが入れ替わる corruption になる。amd64 経路では
  SubProcess を起動せず、Fileinfo.client_socket() を直接呼ぶ。
- **MSG_PEEK は libc/wget で必須** (Phase 27 step 11) — Java の
  Socket.InputStream には peek API が無いので、Fileinfo に peekBuf を
  持たせて「先読み済バイトを次の Read で再消費」させる。peek は本物の
  Linux と違い 1 回 socket から実 read するので、その後の read は
  peekBuf から消費すること。これがないと wget が "HTTP/0.9 mode" に
  切り替わって body parsing が壊れる。
- **AF_UNIX / UDP は EAFNOSUPPORT で fallback を強制** (Phase 27 step 11) —
  glibc は AF_UNIX (nscd) と AF_INET UDP (DNS) を順に試す。実 DNS
  サーバが無いと socket は接続できても UDP send/recv が hang する。
  socket() 自体を `EAFNOSUPPORT` で失敗させると glibc が file 経由
  (`/etc/hosts`) に fall back する。これが無いと curl --version 等が
  起動時に DNS で hang する。
