...
- busybox (静的リンク) で **86 applet** が動作 (Phase 19 サーベイ)。代表:
  sh / ls / cat / echo / grep / sed / tr / tee / wc / head / tail / seq /
  awk / find / sort / uniq / cut / od / hexdump / md5sum / sha256sum /
  base64 / xargs / yes / dd / tar / diff / mkdir / rmdir / rm -rf / cp /
  mv / touch / printf / expr / test / dirname / basename
- パイプライン・fork/exec/wait4 (exit code 伝搬済) ・シグナル配信・
  getdents64・連続リダイレクト・O_APPEND 対応済み
- 回帰テスト 46 本中 45 PASS / 1 SKIP (`sys_chmod64` は WSL DrvFs 制約)
...
1. **busybox 1.36 の SIMD deflate / base64 decode 経路を直す** —
   `gzip`/`gunzip`/`base64 -d` が長い入力で途中切断する。CRC32 / PSHUFB
   / SSSE3 等の不具合が疑われる。`/tmp/busybox-1.30.1/busybox_unstripped`
   と症状を比較しつつ命令単位で詰める
2. **動的リンク対応** (`ld-linux-x86-64.so.2`) — 標準 `/bin/ls`,
...
6. **シグナル拡張**: SIGCHLD/SIGALRM 自動配信、SA_SIGINFO
