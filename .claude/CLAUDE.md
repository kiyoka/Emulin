...
- **動的リンク + 複雑な libc 機能が動作** — printf 各種フォーマット
  ...
  strftime、sscanf、qsort 等 (Phase 25)
...
- **回帰テスト 132 本 PASS / 0 FAIL / 0 SKIP** — sandbox を Linux 側 /tmp に
...
- 動的リンクで hello より複雑な libc 機能 (printf 全フォーマット /
  malloc / setjmp / strftime / sscanf / qsort 等) が動作
- bignum 系の致命バグ (PF 未追跡、ADC/SBB フラグ未更新) を解消
- 回帰 **132 PASS / 0 FAIL / 0 SKIP** (旧 131 + printf_dyn64)
- 未対応のまま: pthread (clone3 #435 + futex)、GCC nested function の
  実行可能スタック、PIE (ET_DYN executable)
