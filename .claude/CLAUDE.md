...
## 現状サマリ (Phase 34-A3 step 9 / basic-block JIT 完了時点)
...
- step 9 (basic-block + block-entry-only): 14.3s → 15.0s (+5%)

current avg block size は 1.11-1.38 insns/block。JIT 対象 opcode が
8 family (MOV r/r, MOV imm64, JMP rel8/rel32, Jcc rel8, RET, CALL rel32)
しか無く、ほとんどの block が「制御転送 1 個」で終わるため。今後 ALU
opcode を追加すれば自然に block が伸びる。
...
- **EMULIN_USE_JIT=1 はまだ default off** — neutral まで来たが net speedup
  には至っていない。次は対応 opcode を増やして block size を伸ばす段階
