...
- **git clone HTTPS は libtasn1 の bogus pointer で segfault** (step 46 で
  真因特定): CA load + ld.so symbol 解決はすべて完璧動作。GOT[free]=0x40282d50
  は **libc の free 関数の正しいアドレス**。free() 内で `mov -0x8(%rdi),%rax`
  (chunk metadata 読み出し) で fault。libtasn1 が `*(node->[0x50])` を
  free しようとしているが、そこに **0x1440 という bogus pointer** が入って
  いる。0x1440 は malloc 直前の chunk size 値で、libtasn1 の構造体 field が
  本来 NULL 初期化されるべきところを uninitialized で残っている可能性大。
  これは libtasn1 の使い方バグというより、emulator の memory init バグの
  可能性 (深掘り必要)
