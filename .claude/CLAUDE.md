...
- **git clone 多経路で動作** (Phase 28-3 で本格対応):
  ...
  - `git clone --no-hardlinks file://...`: 完走 (step 39 CLOEXEC + Phase 28-3
    の `/bin/sh` symlink + Pipeinfo race fix)
  - `git -c http.sslVerify=false clone https://github.com/...`: 完走 (step 41)
  - **`git clone https://github.com/octocat/Hello-World.git`** (cert verify
    あり): **13.5 秒で完走** (Phase 28-3c の `amd64_mremap` 新設で segfault
    解消 + locale + 単一 root cert)
  - **bash → fork+exec で git clone** (Phase 28-3): bash 5.2.21 から
    `git clone --no-hardlinks file://...` / `https://github.com/...` の
    両方が動作
...
- **対話 bash 5.2** + line edit (history / cursor / Tab) — Phase 28-3a で
  `-CJ` を launcher に常時付与、JLine raw mode が tcsetattr ICANON off で
  発動する既存 chain でコード変更ゼロで動作
- **bash + git clone (file:// と https://github.com/...) 両方動作** —
  Phase 28-3c の `amd64_mremap` 新設 + Phase 28-3e の `Pipeinfo` thread-safe
  化で実用化。regression に 4 ケース追加
- **回帰テスト 221 PASS / 0 FAIL / 0 SKIP** (Phase 28-3 で 211 → 221、
  bash 6 + git-clone 4 ケース追加)。3 連続安定。`run-fast.sh` 27 秒、
...
JFR (Java Flight Recorder) で hot spot を特定して最適化:

- **step 60**: `Memory.globalStoreEpoch++` を multi-thread 時のみ実行
  (`Memory.multiThreadActive` を Thread64 が start/finish で inc/dec)
- **step 61**: `Memory.load8` / `store8` を fast/slow path 分離。
  load8 は 698→65 byte で JIT inline 可能サイズに。
  `CacheState.lastSegment` で store fast path に segment cache
- **step 62**: Cpu64 に instruction byte buffer (16 byte prefetch)。
  decode_and_exec の連続 mem.load8(pc) を `fetchInsnByte` (40 byte) で集約
- **step 63**: REX prefix を switch ループ外で先処理して dispatch skip
- **step 64 ★**: JIT compilation log で **`Cpu64::decode_and_exec`
  (= 20,476 byte) が JIT C2 から compile 拒否されている** 真因を発見
  (`HugeMethodLimit` default 8000 byte 制限)。
  `-XX:-DontCompileHugeMethods` flag を `tests/scripts/run-test.sh` と
  `dist/launchers/emulin.{sh,bat}` に追加。

効果:
- git clone HTTPS (cert verify あり): 14.4s → **10.4s (-28%)**
- regression: 211 PASS / 0 FAIL 維持
- run-all: +4% (短時間 test の JIT compile cost、許容範囲)

JIT 化された後 step 60-63 の inline 最適化が効くようになる相乗効果。

---

# Phase 28: ディストリビューション整備 (完了)

5 つの sub-task を完了して、配布まわりの整備を一通り片付けた。

### Phase 28-1: sandbox 整備スクリプト (`dist/build-sandbox.sh`)
host から実機 binary 動作に必要な file を sandbox に配置:
- minimal (~2 MB): busybox のみ
- base (~5 MB): + locale C.utf8 / ld.so.cache / SSL cert / 基本 config /
  /dev/urandom (1 MB) / /etc/gitconfig (sslCAInfo workaround)
- full (~91 MB): + git/curl/openssl/python と必要 .so (ldd で依存解析)

step 58-59 で発見した「locale files / cert files が無いと実機 binary が
動かない」問題を完全に解消。

### Phase 28-5: README 全面更新 (`README.md` / `dist/README.txt`)
- 旧 README は JDK 1.1.6 / cygwin1.dll を参照する legacy 状態 → 現代版に rewrite
- 7 セクションに整理 (クイックスタート / 実機 binary / 性能 / トラブルシュート 等)
- step 64 の `-XX:-DontCompileHugeMethods` 必須を明記

### Phase 28-2: jlink で JRE 同梱 (`dist/build-jre-bundle.sh`)
- jdeps で必要 module を確認 → `java.base` + `java.logging` のみ
- jlink で minimal JRE (~39 MB)
- emulin-jre-<ver>-linux.zip (22 MB compressed) で Java インストール不要

### Phase 28-3: 実機 binary 同梱 demo (`dist/build-demo-bundle.sh`)
- bundled JRE + full sandbox を組み合わせた即実行 demo
- emulin-demo-<ver>-linux.zip (80 MB compressed)
- 解凍直後から `git clone https://github.com/...` が動く

### Phase 28-4: CI / リリース自動化 (GitHub Actions)
- `.github/workflows/ci.yml`: push / PR で軽量 regression (run-fast.sh, ~30 秒)
- `.github/workflows/release.yml`: tag (v*) push で 3 種類 zip を Releases へ
  自動 upload (dist / jre / demo)

### Phase 28-2b: Windows / macOS JRE bundle を CI matrix で生成
- jlink は実行中 JDK と同 platform 用の JRE しか作れない制約に対応
- `release.yml` に matrix [linux/windows/macos] 構成、各 runner で jlink
...
- 副次: bundle 内 emulin.bat を CRLF + ASCII に固定 (LF だと Windows
  cmd.exe が parse 失敗); NOTICE.txt で Temurin OpenJDK の GPLv2+CPE
  ライセンスと source 入手先を明記

### Phase 28-3: bash + git clone を実用化 (a-f)

**a. full sandbox に bash + line edit**
- `build-sandbox.sh` の copy 対象に bash を追加 (libtinfo 依存も自動解決)
- `TIOCSWINSZ` (0x5414) を no-op stub 実装。bash がジョブ起動時に window
  size を伝える ioctl で警告連発していた
- demo launcher (emulin.sh / .bat) に **`-CJ` を常時付与**。bash readline が
  raw byte (ESC sequence) を受け取れば動く既存 chain を活用、コード変更
  ゼロで line edit (history / cursor / Tab) が動作。expect で確認:
  `echo aaa<CR><Up><CR>` → readline が history から `echo aaa` を呼出して再実行

**b. bash → fork+exec git の経路修正 (3 段階)**
1. `/bin/sh` symlink 必須 — git は file:// clone で `/bin/sh -c
   "git-upload-pack ..."` を fork+exec する。sandbox に sh が無いと親 git が
   read で永久 hang する。`build-sandbox.sh` で `/bin/sh → bash` symlink を
   生成
...
3. `protocol.version` は **transport 別** — file:// は v0、https:// は v2
   が必要。global 設定すると片方が壊れる。launcher で env var 強制は廃止、
...
- 真因: `SyscallAmd64.amd64_call` が `syscall #25 (mremap)` で **i386 ABI
  専用の `sys_mremap`** を直接呼んでいた。`sys_mremap` は bx を「stack
  pointer」と解釈して `(int)bx` でキャストし、stack 上の args を
  `mem.load32(base + 4)` で読む i386 仕様
- amd64 では `bx = rdi = old_addr (64-bit pointer)`。`(int)bx = 0xee412000`
  に切り詰め → `mem.load32(0xee412000 + 4)` の int を long に sign-extend
  して `0xffffffffee412004` に access → segfault
- bisect 中、最初は libgnutls 内の bug だと思っていた RIP=0x7ffff7d5337f が、
  実は **libc の syscall 命令 (`0f 05`)** だったと判明 (`rax=0x19` が
  syscall # = 25 = mremap で確定)
...
**d. ★`Pipeinfo` thread-safe 化 — file:// clone の race fix**
- 並列負荷下で `git clone --no-hardlinks file://` が散発的に多様なエラーで
  失敗: `bad line length character: 005?` / `early EOF` /
  `fetch-pack: bad band #48` / `sideband demultiplexer disconnect` /
  `libgcc_s.so.1 must be installed for pthread_exit`
- 真因: `Pipeinfo.read() / write()` が共有 mutable state (buf[]/used/wp/rp)
  を **synchronization なしで update** していた。pthread 後の世界では
  parent git ↔ child git-upload-pack が別 Java thread から同じ Pipeinfo を
  read/write し合うので compound update が racing して data 破損
- 修正: read/write/is_connected/disconnect を `synchronized` 化、
  `sleep + yield` の spin を `wait()` / `notifyAll()` に置換
  (writer は空 reader を、reader は full writer を起こす)
- 効果: parallel 4 連 clone 3/8 PASS → 7/8 PASS、protocol error 完全消滅

**e. regression 4 ケース追加 (`real-coreutils.sh`)**
- `git-clone-file` / `git-clone-content` (直接 clone + 内容 verify)
- `git-clone-bash` / `git-clone-bash-content` (bash 経由 fork+exec clone)
- 211 PASS → **221 PASS**、3 連続実行で安定

**f. bash 自体の regression を 10 → 16 ケースに拡張**
- assoc array、`[[ =~ ]]` regex、substr、function、here-string

## Phase 28 の成果物

```
dist/
  build-dist.sh         旧来の minimal zip (~1.7 MB、busybox + jar、要 system Java)
  build-sandbox.sh      ★ sandbox 構築 (minimal/base/full) — 実機 binary 用
  build-jre-bundle.sh   ★ JRE 同梱 standalone zip (Java 不要)
  build-demo-bundle.sh  ★ JRE + full sandbox demo zip (即実行可)
  launchers/            旧来の launcher (system Java 用)
  README.txt            配布 zip 用 README (全面更新)

.github/workflows/
  ci.yml                CI (push / PR)
  release.yml           Release (tag push で zip 自動 upload)
```

## 配布 zip サイズ早見表

| 種類 | サイズ (zip) | Java | sandbox |
|------|--------------|------|---------|
| dist (busybox) | 1.7 MB | system | 空 |
| jre-bundle | 22 MB | bundled | 空 (busybox のみ) |
| demo | 80 MB | bundled | full (git/curl/openssl 等) |
...
- **Pipeinfo は完全 synchronized + wait/notify** (Phase 28-3e) — read/write が
  共有 mutable state (buf[]/used/wp/rp) を持ち、pthread 後の世界では parent
  process と child process が別 Java thread から同じ Pipeinfo を read/write
  し合う。compound update が racing して `bad line length` / `early EOF` /
  `bad band #N` / `sideband demuxer disconnect` 等多様な data corruption が
  並列負荷下で散発的に発生。read/write/disconnect を synchronized 化し、
  sleep+yield spin を wait/notifyAll に置き換え。git clone --no-hardlinks
  file:// で再現、修正後 protocol error 完全消滅

## syscall ABI 不整合 (i386 vs amd64) の罠

- **`sys_*` は i386 ABI 専用 (stack 経由 args) なので amd64 から直接呼ぶと
  fatal な 32-bit truncation を起こす** (Phase 28-3c) — 例えば
  `Syscall.sys_mremap(long bx, ...)` は `int base = (int)bx` でキャストし
  `arg(base, 0) = mem.load32(base + 4 + 0*4)` で stack を読む i386 仕様。
  amd64 では bx = rdi = 64-bit pointer。`(int)bx` で上位 32-bit を捨て、
  さらに `mem.load32(base + 4)` の int 引数が long に **sign-extend** されて
  `0xffffffff____` になり kernel address にアクセスして segfault。
  amd64 では必ず `amd64_<name>` を作って register 直接引数で書く
- **追跡時の chuvashia**: 当初 RIP 0x7ffff7d5337f が libgnutls 内 instruction
  に見えていたが、実は libc の `0f 05` (syscall) 命令だった。bytes ダンプで
  確定。`rax=0x19` = syscall # 25 = mremap で真因にたどり着いた。
  symfault report の RIP だけ見ると間違った lib だと思い込みやすい
