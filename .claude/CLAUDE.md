...
- **回帰テスト 138 本 PASS / 0 FAIL / 0 SKIP** — sandbox を Linux 側 /tmp に
...
1. **PIE (ET_DYN) の完走** — Phase 25 で 中間状態まで対応済 (ld.so 起動
   までは進む)。ld.so 内の relocation 適用中で 32-bit 切り詰め由来の
   crash が残るので debug 継続。
