...
- **PIE (位置独立実行ファイル) 対応** — `gcc -O0 hello.c` (= デフォルト
  -fpie -pie) で生成される ET_DYN を任意 base にロードして動作
  (Phase 26 step 1)。pie_base = 0x10000000 (32-bit 範囲内) を採用
...
- **回帰テスト 140 本 PASS / 0 FAIL / 0 SKIP** — sandbox を Linux 側 /tmp に
...
- 完全 PIE (`gcc -O0` のデフォルト) が動作
- 動的リンク回帰固定 10 本 (Phase 25 の 8 本 + pie_static / pie_dyn 各 _dyn64)
- 回帰 **140 PASS / 0 FAIL / 0 SKIP** (旧 138 + PIE 2 本)
- 未対応のまま:
    - **64-bit pointer truncation バグの本格修正**
      (現状は pie_base を 32-bit 範囲内にして回避している)。
      8GB 超のヒープが必要な大型プログラムを動かす際に再浮上するので
      その時点で要 debug。
...
1. **64-bit pointer truncation バグの特定と修正** — pie_base を Linux
   典型値 (0x555555554000) に戻せるようにする。再現条件は明確
   (pie_base が 32-bit 範囲を超えるだけ) なので debug 進めやすいはず。
2. **`/bin/ls`, `/bin/bash`, `gcc` 等の本物のバイナリを動かす** —
   PIE が動くようになったので試せる。
3. **pthread 対応** — clone3 真対応 (CLONE_VM 共有メモリ) + 真の futex
   (block/wake) + TLS 拡張。Process / Memory の大改造が必要、工数最大。
4. **socketpair の真の双方向 / AF_INET socket** — Fileinfo に
   read_pipe / write_pipe 二系統を持たせて Linux 互換の双方向通信に。
5. **性能改善**: Decoder 命令キャッシュ復活 / テーブル駆動化
6. **stat 系レイアウトの再確認** (st_atime_nsec の順序など)
7. **シグナル更なる拡張**: SIGALRM 自動配信 (alarm/setitimer)、sigprocmask、
   sa_mask 中ハンドラの再入抑止
8. **dist の充実**: jlink で JRE 同梱の self-contained zip を作る
...
- **PIE 用の load_bias は Linux 典型値に合わせなくてよい** (Phase 26) —
  Linux の典型 PIE base は 0x555555554000 だが、emulator のどこかに
  64-bit pointer の 32-bit truncation バグがあって、そのアドレスだと
  ld.so 内の jump table 解決で crash する。PIE は位置独立なので、
  base を 32-bit 範囲内 (例: 0x10000000) にすれば動く。位置独立性を
  活かして emulator 都合の良い値を選んでよい。
