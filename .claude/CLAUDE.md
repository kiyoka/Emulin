...
- **wget HTTPS は TLS handshake まで動作** (Phase 27 step 15、PSHUFB /
  PALIGNR / PINSRD 実装 + /dev/urandom を S_IFCHR 申告 + HASH-DRBG 切替で
  init pass)。decrypt の段で OpenSSL の **cipher key length 受け渡しの
  根本バグ** (step 8 で記録した DRBG keylen=2048 と同根) で "bad record
  mac" 失敗 — AES 固有ではなく ChaCha20 でも同症状
