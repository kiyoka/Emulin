...
## 現状サマリ (Phase 34-A3 step 8 / JIT 翻訳 PoC 完了時点)
...
### 性能: 5MB sha256sum で 15s → 24s (+66% 悪化)
原因は per-RIP class + INVOKEINTERFACE megamorphic dispatch。
~1000 個の異なる CompiledInsn class が ci.execute(this) call site を
megamorphic 化し、JVM の dispatch コストが optimized interpreter pass
を上回る。Phase 34-A1 の inlining 失敗と同根 (JVM JIT が幾何級数的に
増える小さな class を扱いきれない)。

### 学んだ design 原則 (次 phase で必須)
- **per-RIP 1-instruction class は遅い** — call site が megamorphic 化
  する。最低でも basic-block 単位 (次の制御転送までの命令列) を 1 class
  に詰めて、call site の bimorphic 性を維持する必要あり
- **single-class switch dispatch も検討対象** — 1 つの巨大 class 内で
  RIP → method ID の table jump を持ち、INVOKESTATIC で dispatch する形。
  Java JIT が tableswitch を最適化することは Phase 27 で実証済み (decode_and_exec
  の switch (b0) → C2 jump table)
- **EMULIN_USE_JIT=1 は default off のまま放置** — 単一命令 PoC のままだと
  常に slowdown。次の re-design 後にまた評価
