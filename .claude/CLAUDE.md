...
- **回帰テスト 230 PASS / 0 FAIL** (Phase 28-3 で 211 → 223、Phase 29-B で
  223 → 230、bash 6 + git-clone 6 + python float/stdlib 7)。`run-fast.sh`
  27 秒、`run-network.sh` 3 分、フル `run-all.sh` 約 4m。Phase 29-tty2
  の wait4 yield で +1ms × poll 数の overhead は許容範囲
- **Phase 29-A 最適化** (JFR profile-driven): github HTTPS clone 14.4s →
  **10.2s (-29%)**。
  ...
  線形 scan を skip
- **Phase 29-emacs**: SSE 命令 4 種 (CVTSS2SD / MOVLPS m64,xmm / UNPCKLPS /
  UNPCKHPS) と emacs/python 用 syscall stub (timerfd/signalfd/eventfd/
  epoll_*/pidfd/close_range を ENOSYS、setfsuid/gid 成功、TCSETSF/TCSBRK/
  TCXONC/TCFLSH ioctl)
- **Phase 29-B ★**: `FNSTCW/FNSTSW m16` (D9 /7、DD /7) を 32-bit store から
  16-bit store に修正。read-modify-write で「壊さない」ふりをしていたが直前
  store 後の値を読み戻すため実質 32-bit 書き込み。glibc の `fnstcw -0x2(%rbp)`
  (saved rbp の直前 2 byte) で saved rbp の low 2 byte を 0 で潰し pop %rbp で
  rbp 破壊 → caller の局所 (例: format struct ptr) が無関係領域を指して
  python repr(0.5)="5e-01" や print(0.1+0.2) segfault を起こしていた。修正後
  `print(0.5)`, `math.sqrt(2)`, `print("%.1f" % 0.5)` 等が host python と一致
- 副次成果: minimal sandbox (libc/libm/libz/libexpat + python3.12 stdlib) で
  Python 3.12 が広範に動作。`re` / `json` / `collections` / `enum` /
  `functools` / `math` / `datetime` の stdlib import + 基本動作 OK。FNSTCW
  bug は影響範囲が広く、これら import 経路の深い場所で saved rbp 破壊が
  発火しており Phase 29-B で同時解消した
- **Phase 29-C**: `/dev/null` を null_flag で実装。Kernel.is_exist_device
  ("/dev/null") が "<null>" を返すが Fileinfo.open はそれを literally
  `RandomAccessFile("<null>")` に渡して失敗していた。`<null>` 専用 flag を
  追加し、Read=即 EOF / Write=黙って discard。emacs --batch の stdin
  redirect (/dev/null) 経路がこれで通る
- **Phase 29-D ★**: `sys_access` の X_OK と `open_resolved` の O_DIRECTORY
  を実装。emacs 29 の `(file-directory-p PATH)` は内部的に
  `openat(PATH, O_PATH|O_DIRECTORY|O_CLOEXEC)` (= 0x290000) で directory
  判定するが、O_DIRECTORY (0x10000) を無視して file でも fd を返していた
  ため emacs が file を directory と誤認、find-file が default-directory
  として "path/" に chdir 試行で失敗していた。副次バグの X_OK silent 成功も
  Inode.isExecutable() (S_IXUSR check) で修正
- **emacs-nox 29.3 動作** (Phase 29-emacs/B/C/D 4 連で達成、214 MB sandbox):
  `--version`、`-Q --batch --eval` で elisp 評価、`find-file` でファイル
  読み込み、`insert-file-contents + count-lines + search-forward` まで
  動作。対話モード (TTY init) は未確認
- **Phase 29-vim**: 8-bit ADC/SBB (0x10/0x12/0x18/0x1A) と xattr 系 12
  syscall (#188-#199) を ENOSYS で stub。vim 9.1 が `--version` と
  `-e -s -c '%s/...' -c 'wq'` (ex mode 編集 + 置換 + 保存) で動作。
  `INCLUDE_VIM=1` で demo zip に opt-in 同梱可能 (linux 72→101 MB)
- **Phase 29-tty**: `TCGETS` を tty 判定に修正 (pipe/file fd では -ENOTTY
  を返す)。glibc isatty() が pipe を tty と誤認していたバグ。less が
  pipe stdin で "Missing filename" になる問題が解消、git の default less
  pager が動作。Kernel.java の 1998 年 hardcoded env (LESSCHARSET=
  japanese-sjis / JLESSPLANESET=japanese / TERM=vt100) を utf-8 デフォルト
  + host passthrough にリファクタ
- **Phase 29-tty2**: `wait4(WNOHANG)` で yield + 1ms sleep を挟む。tight
  polling で child thread がスケジュールされず vim の glob 展開で
  E79 を起こしていた fork/exec race の仮 fix
- **Phase 30 ★**: wait4 race の本質 fix。`really_exited =
  exit_flag && !exec_replacing` 判定で OLD/NEW 差し替え window race を
  根絶。仮 fix の sleep を除去し並列回帰 flake 解消。`/dev/tty` →
  `<std>` ルートで vim 対話モードの screen 描画も動作
- **bash デフォルト launcher**: emulin.sh / emulin.bat を `/bin/bash -i`
  デフォルトに変更 (旧 `/bin/busybox ash -i`)。bash 不在の minimal
  sandbox では busybox ash に自動 fallback。JVM heap 4g → 8g (fork+exec
  の memory 蓄積対策)
- **PT_INTERP sandbox resolve** (cross-platform 致命 fix): Phase 24 から
  潜在していた、ELF の PT_INTERP path (`/lib64/ld-linux-x86-64.so.2`)
  を sandbox を介さず host から直接 open する bug。Linux 開発機では
  偶然 host にも同 path があり動作していたが、Windows demo で初めて
  顕在化。`sysinfo.get_native_path()` で sandbox 経由に変更
...
| demo (default) | 72 / 38 / 69 MB | bundled | bash + coreutils + git/curl/wget + less + terminfo |
| demo (INCLUDE_VIM=1) | 101 / 54 / 98 MB | bundled | + vim 9.1 |
| demo (INCLUDE_EMACS=1) | 229 / ~120 / 220 MB | bundled | + emacs-nox 29.3 (重い) |

---

# Phase 29: 性能最適化 + 実用 binary 拡張 + cross-platform fix

## Phase 29-A: HTTPS clone -29% (commit 9427b69, 8d7bb51, 4ed68e3)
JFR profile-driven 最適化で github HTTPS clone 14.4s → 10.2s。
- 29-A1: `Segment.peekbs` を System.arraycopy で bulk copy 化
- 29-A2: `Memory.load8_slow` に lastSegment fast path

## Phase 29-emacs/B/C/D/E: emacs-nox 29.3 動作 (5 連 fix)
- 29-emacs (588d755): SSE 4 命令 + 14 syscall stub
- 29-B ★ (a849c41): FNSTCW/FNSTSW を 16-bit store に修正 (32-bit に拡張で
  saved rbp 破壊。glibc の `fnstcw -0x2(%rbp)` で広範に発生)。Python の
  `print(0.5)`="5e-01"/segfault も同時解消
- 29-C (9196d51): `/dev/null` を null_flag で実装
- 29-D ★ (4adb56e): `sys_access` X_OK と `open_resolved` O_DIRECTORY を
  実装。emacs の file-directory-p が openat(O_PATH|O_DIRECTORY) で
  directory 判定する仕様に対応
- 29-E (c174960): emacs interactive で必要だった Mount empty path guard
  + KDGKBMODE silent

## Phase 29-vim (ffc485d): 8-bit ADC/SBB + xattr stub
- 0x10/0x12 (ADC r/m8/r8) と 0x18/0x1A (SBB r/m8/r8) を実装
- xattr 系 12 syscall (#188-#199) を ENOSYS で stub
- vim 9.1 ex mode (`-e -s -c '%s/...' -c 'wq'`) が動作

## Phase 29-tty/tty2 (5497bc8, 6c1f6e6): TCGETS tty 判定 + wait4 race
- TCGETS で fd が STD/ERR でなければ -ENOTTY (less が pipe stdin で
  「Missing filename」になる bug の真因)
- Kernel.java の 1998 年 hardcoded env を utf-8 + host passthrough に
- wait4 WNOHANG で yield + 1ms sleep (vim glob 展開の fork/exec race)
- launcher: bash デフォルト (旧 ash) + JVM heap 4g → 8g

## Phase 30 ★ (6169e45): vim 対話モード起動 + wait4 race 本質 fix
- ★ wait4 で `exit_flag && !exec_replacing` (= really_exited) を見る
  ように修正。kernel.exec の `OLD.exit_flag=true → pinfo.process=NEW`
  差し替え window で wait4 が OLD を見ると exit_flag=true でも実体は
  NEW がまだ起動していない race。Phase 29-tty2 の sleep workaround は
  この race の仮 fix だったが、really_exited 判定で本質解消
- `/dev/tty` を `<std>` 仮想 path にルート (Kernel.is_exist_device)。
  vim/emacs/less が /dev/tty を直接 open する経路で JLine console と
  同じ stdin/stdout を共有
- env-gated debug flag (TRACE_EXEC, TRACE_OPEN) を static final cache 化。
  hot path から System.getenv() を除去 (HashMap lookup overhead 削減)
- ash-applet-survey TIMEOUT 20s→30s (並列負荷下で flake 軽減)
- 効果: vim interactive で screen 描画 + status line `"file.txt" 1L,
  9B` 出力。glob 展開 19 個が E79 なしで全完了。対話編集 (insert mode
  + :wq) は real TTY 環境が必要 (pipe stdin は dumb fallback)

## Phase 29 cross-platform fix (cbc5a23, 766ccba, 6cbf3a5)
- Windows demo bundle launcher (emulin.bat) で引数なし起動時の PowerShell
  Start-Process が空 ArgumentList で構文 error → 引数有無で分岐
- Windows tar.exe の多段 symlink 展開問題 (rootfs/lib64/ld-linux-x86-64.so.2
  → ../lib/x86_64-linux-gnu/ → ../usr/lib/x86_64-linux-gnu/) で symlink
  チェーンが切れる → Windows zip 内では実 file として埋め込み
- ★ PT_INTERP path を sandbox 経由で resolve。Phase 24 から潜在 (Linux
  host では偶然 / 配下にも同 path 存在で動作、Windows で顕在化)

## Phase 29 build script 拡張
- `INCLUDE_EMACS=1` (1531069), `INCLUDE_VIM=1` (255256e) で opt-in 同梱
- emacs libexec/ コピーで cp -r dst 不存在問題 (cfee2d6)
- less + terminfo を default sandbox に追加 (62b8f0d)
- launcher を bash デフォルトに (35d2df5)
...
- **★ FNSTCW/FNSTSW m16 (D9 /7、DD /7) は厳密に 2 byte だけ store** (Phase 29-B)
  — 32-bit store に「広げる」と直後の 2 byte を 0 で潰す。glibc の
  `fnstcw -0x2(%rbp)` (saved rbp の直前) で saved rbp の low 2 byte を破壊
  → pop %rbp で rbp が 0x...f6a0 → 0x...0000 に変身、caller の局所変数が
  無関係領域を指して python repr(0.5)="5e-01" や segfault。read-modify-write
  で「保護したつもり」になっていたが、直前 store 後の値を読み戻すだけで効果
  なし — **コメントを信じず memory 操作の byte 数を機械的に確認する**
...
- **★ open(O_DIRECTORY) を無視すると emacs が file を dir 誤認** (Phase 29-D)
  — emacs 29 の file-directory-p は `openat(path, O_PATH|O_DIRECTORY|
  O_CLOEXEC)` (= 0x290000) で directory 判定する。stat() ではなく openat()。
  flag 0x10000 が立っていて isDirectory()==false なら -ENOTDIR (-20) を
  返す。無視して fd を返すと find-file が "path/" に chdir 試行で失敗
- **★ access(X_OK) を silent 成功にすると致命** (Phase 29-D) — 0644 file に
  対する file-executable-p が `t` を返し emacs が混乱。Inode.isExecutable()
  (S_IXUSR bit check) を新設して -EACCES を返す。「実装簡単だから 0」は罠
- **`<null>` virtual path は Fileinfo.open に伝わる前提** (Phase 29-C) —
  Kernel.is_exist_device("/dev/null") が `<null>` を返すが、それを literally
  `RandomAccessFile("<null>")` に渡すと FileNotFoundException で fail。
  Fileinfo に null_flag を持たせて Read=0 (EOF)、Write=true (discard)
...
- **build-sandbox.sh full の中身** (Phase 28-3g) — bash + GNU coreutils 30+ +
  grep/sed/awk/file/expr + git/curl/wget + less + terminfo。
  `/bin/sh → bash` symlink 必須
- **★ PT_INTERP は sandbox 経由で resolve せよ** (Phase 29) — ELF の
  PT_INTERP に書かれた絶対 Linux パス (例: `/lib64/ld-linux-x86-64.so.2`)
  を host file system にそのまま渡すと Linux 開発機では偶然動くが、
  Windows host では起動不能。Phase 24 から潜在していた cross-platform
  致命 bug。`sysinfo.get_native_path()` で sandbox 経由に
- **★ Windows tar.exe は多段 symlink を file/dir 種別ミスで壊す** (Phase 29)
  — `rootfs/lib64/ld-linux → ../lib/x86_64-linux-gnu/ld-linux` →
  `rootfs/lib/x86_64-linux-gnu → ../usr/lib/x86_64-linux-gnu/` のような
  2-hop symlink を Windows tar.exe が中間 dir symlink を file symlink
  として展開、最終 file 解決が切れる。critical file (ld.so 等) は
  Windows build で実 file として埋め込む
- **★ wait4 で exec_replacing race を考慮せよ** (Phase 30) — kernel.exec
  は `OLD.exit_flag=true` を `pinfo.process=NEW` 差し替えより前にする
  ため、wait4 がこの window で OLD を見ると「子は終了済」と誤判定して
  ret=pid を返す。child の実体は NEW (= 新 binary、まだ起動前) なので、
  caller が tempfile を読みに行って失敗する (vim glob で E79)。
  `really_exited = exit_flag && !exec_replacing` で判定すること。
  Phase 29-tty2 の Thread.sleep workaround は仮 fix だった真因
- **debug 用 env flag は static final で cache せよ** (Phase 30) — Java
  の System.getenv() は HashMap lookup なので hot path で毎回呼ぶと
  並列負荷下で flake 発生。class 初期化時に 1 回だけ評価して
  `private static final boolean` に保存する
- **★ TCGETS は tty fd に限れ** (Phase 29-tty) — pipe/regular file fd で
  TCGETS が常に成功すると glibc isatty() が pipe を tty と誤認、less が
  「Missing filename」で fail 等の連鎖。STD/ERR 以外には -ENOTTY を
  返すこと
- **JVM heap は fork 連鎖を考慮して大きめに** (Phase 29-tty2) — fork が
  CoW なし memory copy の実装だと、vim glob 展開等の連続 fork+exec で
  memory が積み上がる。launcher は -Xmx8g 程度
