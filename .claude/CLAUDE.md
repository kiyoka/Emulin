...
- **実機 /bin/ls (動的リンク + libc + libselinux + libpcre2) が動作** —
  `ls /` / `ls -la /tmp` / `ls -l /lib` まで動作 (Phase 27 step 1)
...
- **回帰テスト 143 本 PASS / 0 FAIL / 0 SKIP** — sandbox を Linux 側 /tmp に
...
- `tests/scripts/real-ls.sh` — 実機 `/bin/ls` を sandbox にコピーして
  ls / ls -la / ls -l の 3 ケースを Emulin で回す回帰 (Phase 27)。
  host に /bin/ls / ld-linux / libc.so.6 が無ければ SKIP。
...
回帰固定: `tests/scripts/real-ls.sh` で host の `/bin/ls` を sandbox に
コピーして 3 ケース (ls / ls -la / ls -l) を回す。run-all.sh に組み込み。
PIE が動くようになった成果として、まず最初の実機バイナリが「ただ
syscall を埋めるだけ」で通った好例。

## Phase 27 の到達点 (進行中)

- 回帰 **143 PASS / 0 FAIL / 0 SKIP** (Phase 26 の 140 + real-ls 3 本)
- 実機バイナリ第 1 号 = `/bin/ls`
