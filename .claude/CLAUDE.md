...
- **回帰テスト 134 本 PASS / 0 FAIL / 0 SKIP** — sandbox を Linux 側 /tmp に
...
- 動的リンクで hello より複雑な libc 機能 (printf 全フォーマット /
  ...
  wchar / C++ STL + 例外 / fork+pipe IPC 等) が動作
- bignum 系の致命バグ (PF 未追跡、ADC/SBB フラグ未更新) を解消
- 動的リンク回帰固定 4 本 (hello / printf / regex / cpp 各 _dyn64)
- 回帰 **134 PASS / 0 FAIL / 0 SKIP** (旧 130 + 動的リンク 4 本)
- 未対応のまま: pthread (clone3 #435 + 真の futex)、GCC nested
  function の実行可能スタック、PIE (ET_DYN executable)
