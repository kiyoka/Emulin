...
```
  guest process (ring 3, 例: V8)
       │  GVA (guest 仮想アドレス)
       ▼
  ┌─────────────────────────────────────────┐
  │ ① guest page table  (emulin の MMU)       │   pool の PT 領域 [PT_BASE, DATA_BASE)
  │     GVA → GPA を 4-level walk で変換        │   ptNext で bump 割当
  └─────────────────────────────────────────┘
       │  GPA (guest 物理アドレス = pool offset)
       ▼
  ┌─────────────────────────────────────────┐
  │ ② 物理プール  (bump allocator)             │   data 領域 [DATA_BASE, size)
  │     GPA の実体。dataNext が単調増加          │   size = EMULIN_NATIVE_POOL_MB
  └─────────────────────────────────────────┘
       │  WHvMapGpaRange / KVM memslot (SLAT)
       ▼
  ┌─────────────────────────────────────────┐
  │ ③ host 仮想メモリ  (mmap MAP_ANON)          │   ★ここは OS が demand paging
  │     未 touch の host ページは backing されない │      (#304 lazy commit で WHP も commit-on-map)
  └─────────────────────────────────────────┘
       ▼
     host 物理 RAM
```

**重要**: ③(host RAM) は OS の demand paging で lazy。しかし ②(guest 物理プール) の **bump allocator `dataNext` は eager に前進**する。つまり「host RAM は食わないが、guest プールの GPA 空間は食い潰す」。

### 2.2 問題: anonymous mmap の eager 割当

`anonMmap` は mmap 領域の **全ページに即座に GPA を割り当てる**（`allocData` を per-page で呼ぶ）。

```
  V8:  mmap(NULL, 2GB, PROT_NONE, MAP_ANON|MAP_NORESERVE, -1, 0)
                                   └─ Linux ではただの「予約」。物理 0、触れたら demand-zero
        │
        ▼  emulin anonMmap (現状 = eager)
   ┌──────────────────────────────────────────────────────┐
   │ for page in [addr, addr+2GB) step 4KB:   (= 524,288 回)  │
   │     gpa = allocData()      → dataNext += 4KB  ★全部割当   │
   │     mapPage(gva, gpa)      → guest PTE を埋める           │
   └──────────────────────────────────────────────────────┘
        │
        ▼
   dataNext > size (例 2GB) → NativeOom → -ENOMEM (#389)
        │
        ▼
   ★ V8 が実際に触るのは数十 MB なのに、2GB の予約で
     プール全体を使い切って起動失敗
```
...
V8 の cage は `PROT_NONE` で予約され、commit するサブ領域だけ `mprotect(PROT_READ|WRITE)` される。この **mprotect を契機に割り当てる**。

```
  V8:  mmap(NULL, 2GB, PROT_NONE, ...)
        │
        ▼  emulin anonMmap  (PROT_NONE を検出)
   ┌────────────────────────────────────────────┐
   │ region を記録するだけ (mmapRegions)            │
   │ GPA 割当なし / guest PTE は not-present のまま   │  ← ★ここで pool を食わない
   └────────────────────────────────────────────┘
        │
        ▼  後で V8 が commit
  V8:  mprotect(addr+X, 16MB, PROT_READ|PROT_WRITE)
        │
        ▼  emulin sys_mprotect (#10) を hook
   ┌────────────────────────────────────────────┐
   │ この 16MB 分だけ allocData + mapPage           │  ← ★触る分だけ割当
   └────────────────────────────────────────────┘

   pool 消費 = 実 commit 分のみ (数十 MB)
```

- **#PF intercept 不要**（mprotect は syscall 層で完結）。実装は `sys_mprotect`(#10) と `anonMmap` の PROT_NONE 分岐のみ。
...
```
  V8:  mmap(NULL, 2GB, ...)
        ...
        ▼  emulin anonMmap
   ┌────────────────────────────────────────────┐
   │ region 記録のみ、GPA 割当なし、PTE not-present  │
   └────────────────────────────────────────────┘
        │
        ▼  guest が touch (read/write)
   CPU の page-table walk → not-present → #PF 例外
        │
        ▼  guest IDT[#PF] → hlt stub → VM exit
   ┌────────────────────────────────────────────┐
   │ emulin: CR2 = faulting GVA を読む              │
   │   → allocData + mapPage → IRET 相当で resume   │
   └────────────────────────────────────────────┘

   pool 消費 = 実 touch 分のみ (最小、Linux と同一)
```
...
```
  EMULIN_NATIVE_POOL_MB=4096 等に拡大
        │
        ▼
   ★ #379 の制約: 全 pool を host 低位 32GB window に確保
      8GB heap + Σ(プロセス数 × pool) > 32GB → VirtualAlloc2 失敗
        │
        ▼
   V8 予約 2〜4GB × 複数プロセス → window 枯渇 → 破綻
   (しかも eager のまま = 触れないページに物理を払い続ける)
```
...
flowchart TD
  ...
  Q2 -->|"No (eager 残存)"| B
  B --> Done2["✅ 完了 (汎用)"]
  C["戦略 C: pool 拡大"] -.->|"#379 で破綻<br/>= 却下"| X["✗"]
