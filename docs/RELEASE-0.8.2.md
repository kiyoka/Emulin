...
- ~~**#926 (`getaddrinfo` の `EAI_AGAIN`)**~~ … **2026-08-14 にクローズしました。**
  差し替え後の 0.8.2 で Codex を常用して**再発が観測されない**ためです。ただし
  「直した」ではなく**「症状が観測されなくなった」**です: 同型の実在バグ (UDP の先読み
  横取り) は本リリースで直しましたが、症状そのものは再現できておらず、それが原因だったかは
  確認できていません。再発したら `EMULIN_TRACE_NET=1` で DNS の UDP を追ってください。
