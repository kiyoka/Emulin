...
## 6. 次アクション（★2026-06-22 改訂: B 採用なら / A 据置なら）

1. （計測）✅ 完了。perf（§2）+ mmap サイズ分布（§7）+ region 別 touch 密度（§7.1）。結論: 実 workload に dense-large-touch 無し → **ハイブリッド不要**、判断は A/B。
2. **（B を採るなら）WHP 実機 soak**: ① whp-oracle.ps1（stack 保護 + demand paging、本セッションの review fix は KVM のみ）② 主要 demo binary（emacs/vim/git/gcc/python）を default-on で byte-identical & 動作確認。
3. **（B を採るなら）gate 反転**: `NATIVE_PF` default を ON に + `EMULIN_NO_NATIVE_PF=1` で eager 強制の escape hatch。実装は小（3 値化・閾値・bimodal 議論は不要）。
4. **（A 据置なら）何もしない**。default-on 判断を保留。claude 利用者のみ `EMULIN_NATIVE_PF=1`。
5. （任意）node 等でも分布計測すれば B の安全性確証が増す。dense-large workload を実際に走らせる必要が出たら、その時に C（閾値ハイブリッド）を再検討（§7 の閾値議論はその時の出発点）。
