...
- 閾値の目安: alloc_huge（≥2GB）は当然 reserve-only。anon mmap は例えば **≥ 32〜64MB の reserve は demand-paging、それ未満は eager**（多くの small heap/arena を eager に保つ）。閾値は claude/node の cage（≥GB）と一般 malloc arena（≤数 MB）の分布から決める（要追加計測）。
...
1. （計測）claude / node / 一般 coreutils の mmap サイズ分布を `EMULIN_TRACE_MMAP` で採取し RESERVE_THRESHOLD を決める。
