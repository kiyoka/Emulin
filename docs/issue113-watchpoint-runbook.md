...
- **`EMULIN_FORCE_SINGLE_THREAD=1`**: pthread worker の spawn を `-EAGAIN` で拒否し
  真に single-thread 化する (旧実装は worker を spawn したまま cache coherency だけ
  切る逆効果バグ = それ自体が corruption 源だった。これを修正)。**用途は「worker
  無しで #113 が再現するか」の切り分け実験**: emacs が EAGAIN で abort するなら
  worker が crash 経路に必須 / 完走するなら concurrency が原因 / それでも crash する
  なら concurrency は無罪。**注意: 過去の「#113 は single-thread でも再現する」は
  壊れた FORCE_ST 下の観測なので未検証**。native-comp 等で worker 前提の emacs は
  起動しない可能性がある。通常の watchpoint ハントには付けず、切り分け時だけ使う。
