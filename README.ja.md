...
2. **配布 zip をダウンロード**
   ...
   `debian-emulin-0.7.0-windows.zip` を取得します(ローカルでビルドする場合は
...
`debian-emulin-0.7.0-windows.zip` は **Debian 13 (trixie) base 相当**の
...
# 2. Claude Code を「最後の Node.js 版」に固定してインストール
...
launcher `.bat` を置くとワンクリックで起動できます:
...
- AI エージェント固有の制限 (Claude Code のバージョン上限・tmux 等) は
