...
> ★ **既知の不具合 (0.8.1): コマンド実行を伴う依頼が返りません (issue #921)。**
> `setcred` で credential を登録した構成で、**チャット (テキストだけの応答) は正常に返りますが、
> ファイル一覧の取得などコマンド実行を伴う依頼をすると応答が返らなくなります**。モデルが
> 「コマンドを実行します」と答えた直後、実行指示が届かないまま止まります。
> **0.8.2 で修正して出します。** Claude Code はこの事象の影響を受けません。
>
> (同じコマンドを guest の bash から直接実行すれば一瞬で返ります。エミュレータの
> 凍結ではありません。`setcred` を使わない構成で再現するかは未検証です。)
