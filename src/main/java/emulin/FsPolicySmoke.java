...
public final class FsPolicySmoke {
  ...
  /** 子 JVM: env に応じて FsPolicy へ問い合わせ、結果を 1 行で返す。 */
  private static void child() {
    FsPolicy.setRootfs( System.getenv( "T_ROOTFS" ) );
  ...
  private static String ask( String allow, String deny, String rootfs, String... paths )
      throws Exception {
    ...
    pb.environment().remove( "EMULIN_FS_DENY" );
    if( allow != null ) pb.environment().put( "EMULIN_FS_ALLOW", allow );
    if( deny  != null ) pb.environment().put( "EMULIN_FS_DENY",  deny );
  ...
  private static void run( File tmp, File rootfs, File work, File secret, File workish,
    ...
    String r = ask( null, null, rootfs.getPath(), inRoot, inWork, inSec );
    check( "111".equals( r ), "既定は無制限 (env 未設定): " + r );

    // (2) allowlist モード
    r = ask( work.getPath(), null, rootfs.getPath(), inRoot, inWork, inSec, inWorkish );
    ...
    // (3) ★ 脱出경路
    if( linked ) {
      r = ask( work.getPath(), null, rootfs.getPath(), viaLink );
      check( "0".equals( r ), "★ 許可ゾーン内から外を指す symlink を踏めない: " + r );
    }
    r = ask( work.getPath(), null, rootfs.getPath(), dotdot );
    check( "0".equals( r ), "★ .. で外へ出られない: " + r );

    // (4) deny は allow より優先
    r = ask( work.getPath(), work.getPath(), rootfs.getPath(), inWork );
    check( "0".equals( r ), "deny が allow より優先される: " + r );

    // (5) deny だけの運用 (allowlist にしない)
    r = ask( null, secret.getPath(), rootfs.getPath(), inWork, inSec );
    check( "10".equals( r ), "deny だけなら、それ以外は通る: " + r );
  }
}
